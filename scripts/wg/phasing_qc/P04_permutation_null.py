...
def lrt_pvals_vectorized(x1, n1, x2, n2, rho):
    """Vectorized version of model.test_locus's LRT, skipping the
    per-locus scalar optimizer: for a beta-binomial with the SAME shared
    mu closed-form-ish MLE isn't available, so we reuse the model's own
    1-D optimizer per-locus (still fast: ~200k calls take well under a
    minute) rather than re-deriving a vectorized MLE."""
    from ont_asm_caller.model import fit_mu
    from scipy.stats import chi2 as chi2dist

    pvals = np.empty(len(x1))
    for i in range(len(x1)):
        mu1, ll1 = fit_mu(x1[i], n1[i], rho)
        mu2, ll2 = fit_mu(x2[i], n2[i], rho)
        mu0, ll0 = fit_mu(np.array([x1[i], x2[i]]), np.array([n1[i], n2[i]]), rho)
        lr = max(0.0, 2.0 * (ll1 + ll2 - ll0))
        pvals[i] = chi2dist.sf(lr, df=1)
    return pvals
